(function($) {
	...
	pageOpt.events = function(){
		...
		//***********************************立即运行*****************************************//
		$("#grid_table").on("click", ".action-run", function(){
			var jobId=$(this).parent("td").data("id");
			var status=$(this).parent("td").data("status");
			if(status!="01"){
				layer.alert("仅生效中的任务可运行", {
				  skin: 'layui-layer-molv', //样式类名
				  closeBtn: 0
				});
				return;
			}
			layer.confirm('将立即执行一次该任务', {
			  	btn: ['确定','取消'] //按钮
			}, function(){
				$.ajax({
					url:"/framework/runNow",
					type:"post",
					data:{
						jobId:jobId
					},
		...
		$("body").on("click", ".btn-save-task", function(){
			var jobName = $(this).parents("form").find("input[name=jobName]").val();
			var className = $(this).parents("form").find("textarea[name=className]").val();
			var cron = $(this).parents("form").find("textarea[name=cron]").val();
			var description = $(this).parents("form").find("textarea[name=description]").val();
			if(!jobName){
				layer.msg("任务名称不能为空");
				return;
			}
